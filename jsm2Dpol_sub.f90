MODULE JSM_UTILS
  ...
  CONTAINS
    ...
    SUBROUTINE DETECT_DARKFORS_FOLDER(VVAC, POR, DEL0, AB, STATUS, QFILES_PATH, D_TAG)
        ...
        ! Parse DEL0 from D_STR (using either 'd...' or 'pdg...')
        ! IF (D_STR(1:3) == 'pdg') THEN
        !     READ(D_STR(4:), *, IOSTAT=STAT) TMP
        !     IF (STAT /= 0) THEN
        !         PRINT *, '⚠️  Warning: Failed to read DEL0 from pdg-tag.'
        !         STATUS = .TRUE.
        !         RETURN
        !     END IF
        !     DEL0 = TMP
        ! ELSE IF (D_STR(1:1) == 'd') THEN
        !     READ(D_STR(2:), *, IOSTAT=STAT) TMP
        !     IF (STAT /= 0) THEN
        !         PRINT *, '⚠️  Warning: Failed to read DEL0 from d-tag.'
        !         STATUS = .TRUE.
        !         RETURN
        !     END IF
        !     DEL0 = TMP
        ! ELSE
        !     PRINT *, '⚠️  Unknown d-tag format: ', D_STR
        !     STATUS = .TRUE.
        !     RETURN
        ! END IF
    ...
    SUBROUTINE PAH_EMIS
        ...
        INTEGER :: JMAXPW, I, K, IMIN, IMAX, IPWMAX, IRATIO, ITERPW
    ...
    ! c
        ...
        REAL(KIND=8) :: GION, RDG
        INTEGER :: I, II, J, K, ISYN
    ...
    SUBROUTINE PW_ZUSTAND(IMIN, IMAX, IPWMAX, ITERPW)
        ...
        USE abu_mod
        USE rdark_mod  
        INTEGER :: IMIN, IMAX, IPWMAX, ITERPW, I
