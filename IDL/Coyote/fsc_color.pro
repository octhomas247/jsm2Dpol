...
; Always print error message to terminal instead of using a widget
IF StrUpCase(callingRoutine) EQ "$MAIN$" THEN $
   Print, 'Warning: ', theMessage $
ELSE $
   Print, 'Warning from ', callingRoutine, ': ', theMessage
...
ENDELSE
   ...
   ; Add system color names for IDL version 5.6 and higher.
@jismo_noscreen.inc
IF (Float(!Version.Release) GE 5.6) AND (noscreen EQ 0) THEN BEGIN
    ; IF Float(!Version.Release) GE 5.6 THEN BEGIN
   tlb = Widget_Base()
   sc = Widget_Info(tlb, /System_Colors)
   Widget_Control, tlb, /Destroy
