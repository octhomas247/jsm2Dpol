...
; ----------------------------------------------------------------------
function convert_distance, cm=cm, meter=meter, kpc=kpc, Mpc=Mpc
...
; Eq. 13)
    ...
    ez = sqrt(olh.omega0*cube(1.0+z)+omegaR*(1.0+z)*(1.0+z)+olh.omegalambda)
    return, ez
...
; distance modulus (Hogg Eq. 24)
    common cosmology
    conv = convert_distance(_extra=extra)
    return, conv * 5.0d0 * alog10(dluminosity(z) / 10.0d0)
end 
