...
; Parameters of dust model read by a.j. One may use default parameters
; as in Input/jsm12fit*.inp (mpfit convergence is accelerated when
; using 'HD*jsm12fit.inp')
;
; Keywords: ps: plot reddening curve and fit
; to pl_allRedd.pdf no_mpfit: 1=nofit 0=mpfit is used (default)
;
;
; OUTPUT:
...
; read input parameters using the results after mpfit from xdr file:
           file  = findfile('./Result/'+target+'*para.xdr', cou=nf)
           if nf le 0 then stop, ' check results xdr file'
           file  =  file(0)
           restore, file 
...
print, '>> DEBUG: oeRsv defined? ', N_ELEMENTS(oeRsv)
print, '>> DEBUG: rpp   defined? ', N_ELEMENTS(rpp)
print, '>> DEBUG values:', oeRsv, rpp
...
ENDIF ELSE BEGIN
  !p.thick = 2
  !p.charsize = 1.3
  !p.font = 0  ; PostScript font for non-screen
ENDELSE
...
IF keyword_set(ps) THEN BEGIN
  ...
  IF noscreen EQ 0 THEN BEGIN
    a = PSWINDOW(/cm)
    device, filename='idl_redd.ps', BITS_PER_PIXEL=8, /color, $
            XSIZE=a.xsize, YSIZE=a.ysize, XOFFSET=a.xoffset, YOFFSET=a.yoffset
  ENDIF ELSE BEGIN
    device, filename='idl_redd.ps', BITS_PER_PIXEL=8, /color, XSIZE=15, YSIZE=10
  ENDELSE
ENDIF
...
;                  restore, fxdr
  ...
  fredd = findfile('../../Data/Data_Redd2023_AA676_132_Tab4/'+target+'*.redd', cou=nfr)
  fredd = fredd(0)
...
; save jsmTaufit.inp read by a.j (Note: tauV not Av)
   
   p_serk = 1.                  ; optical polarisation as max of Serkowski fit not used here set it arbritarily to 1%
...
parain(7).limits(0) =  4.4d-6          ; arad_polmin_aC
parain(7).limits(1) =  2.6d-5  ; 
parain(8).limits(0) =  4.4d-6          ; arad_polmin_Si
...
rdfloat, './Output/Kappa.out', skipline=3, w, sac, ssac, sasi, $
 ...
 areddmod  = 2.5/alog(10.) * (st/st(kvis) -1.) * st(kvis) 
...
print, ' relative reddening E(x-V)/Ebv is saved'
 ...
 IF keyword_set(ps) THEN BEGIN
    ...
    spawn, 'ps2pdf idl_redd.ps pl_allRedd.pdf'
    print, 'ps2pdf idl_redd.ps pl_allRedd.pdf     ->      DONE'
  ENDIF
